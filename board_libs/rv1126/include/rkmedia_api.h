 ...
 * TCS api
 ********************************************************************/
typedef void (*tcs_Join_callback)();
typedef void (*tcs_timeout_callback)(RK_U32);
typedef void (*tcs_push_callback)(const char*, int, const char *);
typedef void (*tcs_pop_callback)();
typedef void (*tcs_count_callback)();
typedef void (*tcs_leave_callback)();

_CAPI RK_VOID RK_MPI_TCS_SetJoinCallback(tcs_Join_callback);
_CAPI RK_VOID RK_MPI_TCS_SetTimeoutCallback(tcs_timeout_callback);
_CAPI RK_VOID RK_MPI_TCS_SetPushCallback(tcs_push_callback);
_CAPI RK_VOID RK_MPI_TCS_SetPopCallback(tcs_pop_callback);
_CAPI RK_VOID RK_MPI_TCS_SetCountCallback(tcs_count_callback);
_CAPI RK_VOID RK_MPI_TCS_SetLeaveCallback(tcs_leave_callback);


extern tcs_Join_callback tcs_join;
extern tcs_timeout_callback tcs_timeout;
extern tcs_push_callback tcs_push;
extern tcs_pop_callback tcs_pop;
extern tcs_count_callback tcs_count;
extern tcs_leave_callback tcs_leave;
...
#define RK_TCS_SETTIMEOUT(a) \
    if (tcs_timeout) \
        tcs_timeout(a); \
    else { \
    }
#define RK_TCS_PUSH(func)  \
    if (tcs_push) \
        tcs_push(__FILE__, __LINE__, func); \
    else { \
    }
#define RK_TCS_POP \
    if (tcs_pop) \
